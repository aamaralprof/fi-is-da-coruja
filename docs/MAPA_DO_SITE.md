...
- Dados: estado JSON retrocompatível com `appearance` (pacote, cordão de luzes e o tapete, que além de ligado/desligado guarda posição em `rugX`/`rugY`), `roomItems` (posição normalizada, estado, exibição e, na planta, `lastWatered`) e painéis por caso (`itens`, `ligacoes`, `nota`, `conclusao`). A configuração central de `sala.js` define zona/perspectiva, asset, tamanho, posição inicial e estados dos objetos. O Worker valida tipo e limite de `appearance` e `roomItems`, e aceita sem exigir os campos do formato antigo.
