...
- A linha do tempo mantém histórico para o botão **Desfazer** durante a montagem e reconstrói uma sequência progressiva de estados quando uma atividade já concluída é restaurada, permitindo rever a ordem sem revogar o progresso já desbloqueado.
- A pesquisa de Sísifo no Explorador usa os rabiscos e destaca o desenho correspondente, sem antecipar nenhuma das imagens do quebra-cabeça. O quebra-cabeça é uma interação independente, aberta por um card próprio, e funciona por arraste ou seleção de duas peças. A imagem das peças é pré-carregada e o botão informa o carregamento antes de abrir o modal, evitando o tabuleiro branco em conexões mais lentas. Os recortes usam a mesma ordem espacial da validação, de modo que a imagem visualmente montada é exatamente a combinação aceita. Um botão de conferência permanece visível sob o tabuleiro: enquanto a montagem estiver incorreta, informa que ainda há peças fora do lugar; quando estiver correta, passa a **Revelar o registro da Ordem** e somente então troca para o Sísifo da Ordem. A imagem de revelação é mantida no DOM para evitar recarga e travamento. As pesquisas das Moiras e da coruja também destacam os desenhos correspondentes com movimento reduzido acessível.
...
- Dados: estado JSON retrocompatível com `appearance` (pacote, cordão de luzes e o tapete, que além de ligado/desligado guarda posição em `rugX`/`rugY`), `roomItems` (posição normalizada, estado, exibição e, na planta, `lastWatered`), painéis por caso (`itens`, `ligacoes`, `nota`, `conclusao`) e `enigma` (`book`/`notebook`/`postit`/`completed`, booleanos). A configuração central de `sala.js` define zona/perspectiva, asset, tamanho, posição inicial e estados dos objetos. O Worker valida tipo e limite de `appearance`, `roomItems` e `enigma`, e aceita sem exigir os campos do formato antigo.
