...
- O quebra-cabeça de Sísifo é opcional, tem um único acesso dentro da pesquisa de Sísifo e funciona por arraste ou seleção de duas peças. Ao ser concluído, o estado resolvido impõe diretamente e mantém visível o Sísifo da Ordem; ao reabrir um quebra-cabeça já resolvido, a revelação reaparece sem depender de temporizadores ou do carregamento da imagem. A imagem de revelação é mantida no DOM para evitar recarga e travamento a cada troca. As pesquisas das Moiras e da coruja destacam o desenho correspondente com movimento reduzido acessível.
