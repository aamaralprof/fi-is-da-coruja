...
const viewData={
 overview:{title:'Atmosfera da sala',description:'Escolha uma área para se aproximar ou mude a atmosfera.',scenes:{lilas:'assets/sala/sala-geral.png',azul:'assets/sala/sala-geral-azul.png'},alt:'Visão geral da Sala de Investigação'},
 shelf:{title:'Objetos da estante',description:'Escolha na bandeja e organize livros e a caixa Ideias nas prateleiras.',scenes:{lilas:'assets/sala/sala-estante-lilas.png',azul:'assets/sala/sala-estante-azul.png'},alt:'Estante aproximada da Sala de Investigação'},
 desk:{title:'Objetos da mesa',description:'Escolha na bandeja e organize os objetos sobre a mesa.',scenes:{lilas:'assets/sala/sala-mesa-aproximada.png',azul:'assets/sala/sala-mesa-azul.png'},alt:'Mesa aproximada da Sala de Investigação'},
 board:{title:'Mural de pistas',description:'Organize pistas, aproxime ideias e trace conexões.',scenes:{lilas:'assets/sala/sala-mesa-aproximada.png',azul:'assets/sala/sala-mesa-azul.png'},alt:'Mural da Sala de Investigação'}
};
const roomItems=[
...
if(progresso('sofia-room-notebook-unlocked')==='unlocked')roomItems.push({id:'notebook_investigacao',name:'Notebook de Investigação',view:'desk',image:'assets/sala/notebook-investigacao.png',x:23,y:57,w:22,ratio:1.78});
...
const sceneFor=meta=>meta.scenes?.[state?.appearance?.wall||'lilas']||meta.scene;
const defaults=()=>({appearance:{pack:'sofia',wall:'lilas',rug:false,rugX:13,rugY:67,lights:'on'},roomItems:{},paineis:{}});
const normalize=s=>{
 s=s||{};s.appearance=s.appearance||{rug:false,lights:'on'};s.appearance.pack=s.appearance.pack||'sofia';s.appearance.wall=s.appearance.wall||'lilas';s.appearance.rugX=Number.isFinite(s.appearance.rugX)?s.appearance.rugX:13;s.appearance.rugY=Number.isFinite(s.appearance.rugY)?s.appearance.rugY:67;s.roomItems=s.roomItems||{};s.paineis=s.paineis||{};
...
$('reset-view').onclick=()=>{if(view==='overview')state.appearance={...state.appearance,wall:'lilas',rug:false,rugX:13,rugY:67,lights:'on'};else for(const d of roomItems.filter(i=>i.view===view))state.roomItems[d.id]={x:d.x,y:d.y,state:d.states?.[0]||'default',placed:!!d.defaultPlaced};selectedRoom=null;renderRoom();renderTray();change();};
