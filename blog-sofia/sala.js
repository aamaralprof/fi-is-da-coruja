...
const defaults=()=>({appearance:{pack:'sofia',wall:'lilas',rug:false,lights:'on'},roomItems:{},paineis:{}});
const normalize=s=>{
 s=s||{};s.appearance=s.appearance||{rug:false,lights:'on'};s.appearance.pack=s.appearance.pack||'sofia';s.appearance.wall=s.appearance.wall||'lilas';s.roomItems=s.roomItems||{};s.paineis=s.paineis||{};
...
$('reset-view').onclick=()=>{if(view==='overview')state.appearance={...state.appearance,wall:'lilas',rug:false,lights:'on'};else for(const d of roomItems.filter(i=>i.view===view))state.roomItems[d.id]={x:d.x,y:d.y,state:d.states?.[0]||'default',placed:!!d.defaultPlaced};selectedRoom=null;renderRoom();renderTray();change();};
